...
#Trying PowerShell Lambda with Boto
def CallPowerShellLambda(body):
    lambdaClient = boto3.client('lambda')
    payload = {}
    payload["message"] = body

    response = lambdaClient.invoke(
        FunctionName="arn:aws:lambda:eu-west-1:535132083307:function:PowerShellLambda-PowerShellPost-1S3NWQ8QDUHLN",
        InvocationType='Event',
        LogType='Tail',
        Payload=json.dumps(payload),
    )
    print("Invokeing PowerShell lambda")

CallPowerShellLambda(body)


#Trying powershell lambda api
uri = "https://b4p0g4pa7h.execute-api.eu-west-1.amazonaws.com/Prod/send"
response = requests.post(uri , json=body)
if response.status_code == 204:
    print('PowerShell Lambda Email sent successfully')
else:
    print('An error occurred sending reaching the PowerShell lambda.')
    print(response)
